"""
Zerodha Integration Tests - Real API Connection

Tests with actual Zerodha KiteConnect API.
...
class TestZerodhaAuthentication:
    """Real authentication tests with Zerodha API."""
    
    def test_auth_01_connect_and_authenticate(self, zerodha_adapter):
        """
        TC_AUTH_01: Real connection to Zerodha API
        
        Expected: Successfully connected and authenticated
        """
        # Adapter is already connected by fixture
        assert zerodha_adapter.is_connected() is True
        assert zerodha_adapter._kite is not None
        logger.info("✓ TC_AUTH_01: Successfully authenticated with Zerodha")
    
    def test_auth_02_get_profile(self, zerodha_adapter):
        """
        TC_AUTH_02: Fetch user profile
        
        Expected: Returns profile with user ID
        """
        profile = zerodha_adapter._kite.profile()
        
        assert profile is not None
        assert 'user_id' in profile or 'email' in profile
        logger.info(f"✓ TC_AUTH_02: Profile retrieved - User: {profile.get('user_id', 'N/A')}")
    
    def test_auth_03_reconnection(self, zerodha_credentials, contract_manager):
        """
...
class TestZerodhaOrdersAMO:
    """AMO order tests - safe for testing."""
    
    @pytest.mark.amo_order
    def test_order_01_place_amo_market_buy(self, zerodha_adapter, amo_order_helper, 
        ...
        # Place order
        response = amo_order_helper.place_and_track(zerodha_adapter, order_request)
    ...
    @pytest.mark.amo_order
    def test_order_02_place_amo_limit_sell(self, zerodha_adapter, amo_order_helper,
                                           test_instruments, skip_if_market_open):
        """
        ...
        ltp = zerodha_adapter.get_ltp(
            underlying=test_instruments['underlying'],
            option_type='PE',
            strike=test_instruments['strike'],
            expiry=zerodha_adapter.contract_manager.get_options_expiry('current_week')
        )
    ...
    def test_order_03_invalid_symbol(self, zerodha_adapter):
        """
        ...
        response = zerodha_adapter.place_order(order_request)
    ...
    @pytest.mark.amo_order
    def test_order_04_cancel_amo_order(self, zerodha_adapter, amo_order_helper,
                                       test_instruments, skip_if_market_open):
        """
        ...
        place_response = zerodha_adapter.place_order(order_request)
        assert place_response.success is True
        
        order_id = place_response.order_id
        
        # Cancel order
        cancel_response = zerodha_adapter.cancel_order(order_id)
...
class TestZerodhaMarketData:
    ...
    def test_quote_01_get_ltp_single(self, zerodha_adapter, test_instruments):
        """
    ...
    def test_quote_02_get_full_quote_multiple(self, zerodha_adapter, test_instruments):
        """
        ...
        quote_ce = zerodha_adapter.get_quote(
            underlying=test_instruments['underlying'],
            option_type='CE',
            strike=test_instruments['strike'],
            expiry=expiry
        ...
        quote_pe = zerodha_adapter.get_quote(
            underlying=test_instruments['underlying'],
            option_type='PE',
            strike=test_instruments['strike'],
            expiry=expiry
    ...
    def test_quote_03_historical_data_1day(self, zerodha_adapter, test_instruments):
        """
        ...
        # Use data feed for historical data
        if hasattr(zerodha_adapter, '_data_feed') and zerodha_adapter._data_feed:
            # Get spot token for NIFTY
            if test_instruments['underlying'] == 'NIFTY':
                ...
                try:
                    # Use Kite's historical data API
                    candles = zerodha_adapter._kite.historical_data(
                    ...
                    logger.info(f"✓ TC_QUOTE_03: Historical data retrieved - {len(candles)} daily candles")
                    logger.info(f"  Latest: Date={first_candle['date']}, "
                              f"Close=₹{first_candle['close']:.2f}")
                    
                except Exception as e:
                    pytest.skip(f"Historical data not available: {e}")
        else:
            pytest.skip("Data feed not available for historical data test")
    
    def test_get_spot_price(self, zerodha_adapter, test_instruments):
        """Test spot price retrieval."""
        spot_price = zerodha_adapter.get_spot_price(test_instruments['underlying'])
...
class TestZerodhaPositions:
    """Position and holdings tests."""
    
    def test_pos_01_fetch_net_positions(self, zerodha_adapter):
        """
        TC_POS_01: Fetch net positions
        
        Expected: Returns list of open positions (may be empty)
        """
        positions = zerodha_adapter.get_positions()
    ...
    def test_pos_02_fetch_holdings_funds(self, zerodha_adapter):
        """
        TC_POS_02: Fetch holdings/funds
        
        Expected: Returns dict with cash, equity holdings
        """
        funds = zerodha_adapter.get_funds()
    ...
    @pytest.mark.amo_order
    def test_pos_03_square_off_position(self, zerodha_adapter, amo_order_helper,
                                       test_instruments, skip_if_market_open):
        """
        ...
        buy_response = amo_order_helper.place_and_track(zerodha_adapter, buy_request)
        ...
        sell_response = amo_order_helper.place_and_track(zerodha_adapter, sell_request)
...
class TestZerodhaWebSocket:
    ...
    @pytest.mark.slow
    def test_ws_01_subscribe_live_ticks(self, zerodha_adapter, test_instruments):
        """
        TC_WS_01: Subscribe to live ticks
        
        Expected: Receives modeFull tick data every ~200ms
        """
        pytest.skip("WebSocket test requires market hours - implement when needed")
        
        # TODO: Implement WebSocket subscription
        # This requires KiteTicker from kiteconnect
    ...
    @pytest.mark.websocket
    def test_ws_02_error_invalid_token(self, zerodha_adapter):
        """
        TC_WS_02: Error on invalid token
        
        Expected: Logs error, reconnects automatically
        """
        pytest.skip("WebSocket error handling test - implement when needed")
    
    @pytest.mark.websocket
    def test_ws_03_unsubscribe_mode(self, zerodha_adapter):
        """
